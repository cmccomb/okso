...
1. Ensure `jq`, `pandoc`, `xmllint` (`libxml2`), `ripgrep-all` (`rga`), `poppler` (`pdftotext`), `docx2txt`, `xlsx2csv`, and a `llama.cpp` binary are on your `PATH` (macOS includes `mdfind` for Spotlight-backed searches).
