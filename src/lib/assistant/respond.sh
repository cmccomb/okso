...
ASSISTANT_RESPOND_DIR=$(cd -- "$(dirname "${BASH_SOURCE[0]}")" && pwd)

# shellcheck source=../core/logging.sh disable=SC1091
source "${ASSISTANT_RESPOND_DIR}/../core/logging.sh"
# shellcheck source=../cli/output.sh disable=SC1091
source "${ASSISTANT_RESPOND_DIR}/../cli/output.sh"
# shellcheck source=../prompt/build_final_answer.sh disable=SC1091
source "${ASSISTANT_RESPOND_DIR}/../prompt/build_final_answer.sh"
# shellcheck source=../schema/schema.sh disable=SC1091
source "${ASSISTANT_RESPOND_DIR}/../schema/schema.sh"
# shellcheck source=../llm/context_budget.sh disable=SC1091
source "${ASSISTANT_RESPOND_DIR}/../llm/context_budget.sh"
# shellcheck source=../llm/llama_client.sh disable=SC1091
source "${ASSISTANT_RESPOND_DIR}/../llm/llama_client.sh"
...
respond_text() {
	...
	log "INFO" "Invoking llama inference" "$(printf 'tokens=%s schema=%s' "${number_of_tokens}" "${concise_schema_path}")" >&2
        ...
        executor_model_repo="${EXECUTOR_MODEL_REPO:-${REACT_MODEL_REPO:-}}"
        executor_model_file="${EXECUTOR_MODEL_FILE:-${REACT_MODEL_FILE:-}}"
        executor_cache_file="${EXECUTOR_CACHE_FILE:-${REACT_CACHE_FILE:-}}"
