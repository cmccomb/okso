...
finalize_executor_result() {
	...
	# Evaluate final answer if enabled and not already evaluated
	if [[ "${ENABLE_ANSWER_VALIDATION:-true}" == "true" ]]; then
		if [[ "$(json_state_get_key "${state_name}" "final_answer_evaluated")" != "true" ]]; then
			evaluate_and_optionally_replan "${state_name}" "${final_answer}" "true"
			return $?
		fi
	fi
...
evaluate_and_optionally_replan() {
	...
	evaluation_fallback_json() {
		# Produces a deterministic evaluator payload that preserves the current answer.
		# Arguments:
		#   $1 - fallback reasoning
	...
	if ((evaluation_status_code != 0)); then
		...
		evaluation_json="$(evaluation_fallback_json "Evaluator unavailable; using tool output.")"
	fi
	...
	if [[ "${evaluation_type}" != "FINAL" && "${evaluation_type}" != "REPLAN" ]]; then
		log "WARN" "Evaluator returned non-conforming output; using tool output" || true
		evaluation_json="$(evaluation_fallback_json "Evaluator returned non-conforming output; using tool output.")"
		evaluation_type="FINAL"
		reasoning="$(jq -r '.reasoning' <<<"${evaluation_json}")"
		output="${final_answer}"
	fi
